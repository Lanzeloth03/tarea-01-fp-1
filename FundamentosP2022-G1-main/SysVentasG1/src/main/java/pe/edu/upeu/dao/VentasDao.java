...
public class VentasDao extends AppCrud{
    ...
    public void carritoVenta(VentaTO vtox) {
        ...
        do {
            ...
            vdTo.setPrecioUnit(Double.parseDouble(String.valueOf(dataPX[0][5]))+Double.parseDouble(String.valueOf(dataPX[0][6])));
    ...
    public void reporteVentas() {
        ...
        try {
            for (int i = 0; i < dataP.length; i++) {
                String[] fechaSeparada=String.valueOf(dataP[1][2]).split(" ");
                Date fechaV=formatFecha.parse(fechaSeparada[0]);
                if (
                    (fechaV.after(formatFecha.parse(fechaI)) || fechaSeparada[0].equals(fechaI)) &&
                    (fechaV.before(formatFecha.parse(fechaI)) || fechaSeparada[0].equals(fechaf)) 
                    ) {
                        cantidadFilas++;
                }
            }
            ...
            System.out.printf("%s %s %5s %10s %s %3s %3s %4s %n","#", "IdV", "DNI",
            "fecha", "DescT", "P.Base", "IGV", "P.Total");
            int num=0;

            for (VentaTO ventaTO : ventasERF){
                System.out.printf("%d %s %5s %10s %3f %3f %3f %4f %n",
